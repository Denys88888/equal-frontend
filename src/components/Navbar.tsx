...
export default function Navbar({ title, showBack = false, onBack, rightAction, transparent = false }: NavbarProps) {
  ...
  return (
    <motion.nav
      ...
      style={{
        backgroundColor: transparent ? 'transparent' : 'rgba(247, 244, 238, 0.85)',
        backdropFilter: transparent ? 'none' : 'blur(12px)',
        paddingTop: 'env(safe-area-inset-top)',
        height: 'calc(56px + env(safe-area-inset-top))',
        borderBottom: transparent ? 'none' : '1px solid rgba(232, 226, 216, 0.6)',
      }}
    >
      ...
      {title && (
        <h2 className="flex-shrink-0 text-xl font-semibold text-[#232323] tracking-tight" style={{ fontFamily: "'Outfit', system-ui, sans-serif" }}>
          {title}
        </h2>
      )}
